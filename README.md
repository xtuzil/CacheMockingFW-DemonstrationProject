...
- create mock - it generate class definition
    ```
    MOCKFW>CreateMock(NameOfTheMock) 
    ```
- export mock for Docker usage 
    ```
    MOCKFW>ExportMock(NameOfTheMock)
    ```
- create mocks and their mocked methods from .csv file (csv file in format: 'Name of the Mock;Method;Input obj;Response;Rest method' )
    ```
    MOCKFW>MockMethodsFromCSV(FilePath)
    ```
- handling mock
    ```
    MOCKFW>GetAllMocks(), GetAllMethods(NameOfTheMock), ExportMockToCSV(NameOfTheMock), DeleteMock(NameOfTheMock), DeleteMethod(NameOfTheMock, NameOfTheMethod), DeleteMethodWithParametr(NameOfTheMock, NameOfTheMethod, Parameters), CleanAll())
    ```
...
- SaveMethod(NameOfTheMethod, params, returnValue, *restMethod)
    ...
    ```
    e.g. MOCKFW>do ##class(ZK.Mocks.Test).SaveMethod("test", "{"Halo":"Ano"}", "return")
    ```
- To get responses from mocked methods of mock, just call name method on the mock with params.
    ```
    e.g. MOCKFW>write ##class(ZK.Mocks.Test).test("{"Halo":"Ano"}")
    ```
...
- in Caché:
    ```
    MOCKFW>MockManager.ExportMock(mockName, dirPath)
    ```
